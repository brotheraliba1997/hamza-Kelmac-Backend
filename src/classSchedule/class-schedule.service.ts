...
export class ClassScheduleService {
  ...
  async joinClass(securityKey: string) {
    ...
    return {
      message: 'Class joined successfullyss',
  ...
  async checkSchedule(data: any) {
    ...
    const courseIds = Array.isArray(coursesInput)
      ? coursesInput
      : [coursesInput];

    // Courses are IDs - fetch from DB with sessions
    let courses = coursesInput;
    if (
      courseIds.length > 0 &&
      (typeof courseIds[0] === 'string' ||
        courseIds[0] instanceof Types.ObjectId)
    ) {
      courses = await this.courseModel
        .find({
          _id: {
            $in: courseIds.map((id) => new Types.ObjectId(id.toString())),
          },
        })
        .lean();
    }

    // Build (course, sessionId) pairs for matching sessions
    const schedulePayloads: Array<{
    ...
    for (const course of courses || []) {
      ...
      for (const session of courseSessions) {
        const sessionIdStr =
          session?._id?.toString?.() || session?.toString?.();
        if (sessionIdStr && sessionIds.includes(sessionIdStr)) {
    ...
    console.log(schedulePayloads);

    const studentIds = Array.isArray(students)
      ? students.map((s) => new Types.ObjectId(s.toString()))
      : [new Types.ObjectId(students.toString())];

    const created = await this.classScheduleModel.insertMany(
      schedulePayloads.map((payload) => ({
        ...
        duration,
        googleMeetLink,
        securityKey: securityKey || randomUUID(),
        status: status || 'scheduled',
        progress: progress ?? 0,
        startedAt,
        endedAt,
        ClassLeftList: studentIds.map(() => false),
      })),
    );

    return created.map((doc) => this.map(doc.toObject()));
  }
}
