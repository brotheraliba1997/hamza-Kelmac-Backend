...
import { StudentItemGrade } from '../student-item-Grade/schema/student-item-grade.schema';
import { playgrouping_v1alpha1 } from 'googleapis';
...
export class AttendanceService {
  ...
  private async getStudentItemGradeResult(courseId: string, studentId: string) {
    ...
    return {
      message: passFailStatus >= 70 ? 'Pass' : 'Fail',
  ...
  async bulkMark(
    ...
    for (const studentAttendance of dto.students) {
      ...
      if (populated) {
        ...
        const populatedData = this.map(populated);
        const student = populatedData.student as any;
        const course = populatedData.courseId as any;
        ...
        if (studentEmail) {
          ...
          const sessionId = populatedData.sessionId
            ? typeof populatedData.sessionId === 'string'
              ? populatedData.sessionId
              : populatedData.sessionId.toString()
            : dto.sessionId;
          const status = populatedData.status || studentAttendance.status;

          await this.mailService.attendanceMarked({
            to: studentEmail,
            data: {
              studentName,
              courseTitle,
    ...
    if (studentIds.length > 0) {
      await this.notificationModel.create({
        receiverIds: studentIds.map((id) => new Types.ObjectId(id)),
        type: 'Attendance Marked',
        title: 'Attendance Marked',
        message: 'Attendance has been marked',
        meta: { attendance: records.map((record) => record.id) },
      });

      const instructorIdStr = instructorId?.toString?.() ?? instructorId;
      const teacherEmail = instructorIdStr
        ? await this.usersService.findById(instructorIdStr as string)
        : null;

      if (teacherEmail?.email && records.length > 0) {
        const firstRecord = records[0];
        const courseData = firstRecord.courseId as any;
        const courseTitle = courseData?.title || 'Course';

        const studentNames = records
          .map((r) => {
  ...
  private mapPassFailRecord(doc: any): PassFailRecordEntity {
    if (!doc) return undefined as any;
    console.log('doc==>', doc, doc?.courseId?.sessions[0]);
    const sanitized = sanitizeMongooseDocument(doc);
    if (!sanitized) return undefined as any;
  ...
  async getPassFailRecordsByStudent(
    ...
    const paginated = await buildMongooseQuery({
      ...
      populateFields: [
        ...
        {
          path: 'courseId',
          select: 'title description sessions',
          populate: {
            path: 'sessions',
    ...
    const dataWithGrades = await Promise.all(
      paginated.data.map(async (record) => {
        const courseId = record.courseId as any;
        const courseIdValue =
        ...
        const assessmentResult = await this.getStudentItemGradeResult(
          courseIdValue,
          studentId,
        );
        ...
        return {
          ...record,
          assessmentResult,
        } as PassFailRecordEntity & { assessmentResult: any };
      }),
    );
    // console.log('dataWithGrades==>', paginated.data[0].courseId.sessions[0]);
    return {
      ...paginated,
      data: dataWithGrades,
    };
  ...
  async approvePassFailStatus(
    ...
    if (approve) {
      ...
      if (
        ...
        // Fetch student for certificate notification email
        const certStudent = await this.userModel
          .findById(record.studentId)
          .select('firstName lastName email')
          .lean();
        const certCourse = await this.courseModel
          .findById(record.courseId)
          .select('title')
          .lean();
        ...
        if (certStudent?.email) {
          const certStudentName =
            certStudent?.firstName && certStudent?.lastName
              ? `${certStudent.firstName} ${certStudent.lastName}`
              : certStudent?.email || 'Student';
          const certCourseTitle = certCourse?.title || 'Course';

          await this.mailService.studentPassFailResult({
            to: certStudent.email,
            data: {
      ...
      // Fetch student and course data for email
      ...
      if (studentEmail) {
        const studentName =
          student?.firstName && student?.lastName
            ...
            : student?.email || 'Student';
        const courseTitle = course?.title || 'Course';
        const isPass = record.status === PassFailStatusEnum.PASS;

        // Send email for both PASS and FAIL cases
        await this.mailService.studentPassFailResult({
          to: studentEmail,
          data: {
            studentName,
      ...
      // If rejecting, clear approval data and send email
      const student = await this.userModel
        .findById(record.studentId)
        .select('firstName lastName email')
        .lean();
