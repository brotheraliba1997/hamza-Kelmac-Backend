...
export class PaymentService {
  ...
  async createPayment(userId: string, createPaymentDto: CreatePaymentDto) {
    ...
    try {
      ...
      try {
        ...
        const student = booking.studentId as {
          firstName?: string;
          lastName?: string;
          email?: string;
        } | null;
        const course = booking.courseId as { title?: string } | null;
        ...
        if (studentEmail) {
          await this.mailService.paymentConfirmation({
            ...
            data: {
              ...
              paymentMethod: PaymentStatus.SUCCEEDED,
  ...
  async confirmPayment(paymentIntentId: string) {
    ...
    try {
      const userId =
        user?._id?.toString() ||
        (typeof user === 'string' ? user : payment.userId?.toString());
      if (userId) {
        await this.notificationModel.create({
          receiverIds: [new Types.ObjectId(userId)],
